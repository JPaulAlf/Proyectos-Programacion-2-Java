...
public class ReciboElectricidad {
    ...
    public int cantidadKwh(){
        if(this.lectauraAnterior<=10000 && this.lecturaActual<=10000){
            int calculoCantidad=(10000-this.lectauraAnterior)-this.lecturaActual;
    ...
    public float montoKwh(){
        if(this.cantidadKwh()<=250){
            if(medidor.getTipo().equals(medidor.getTipo().Comercial)){
                return (float)(this.cantidadKwh()*(1.5*30));
            }else{
                return this.cantidadKwh()*30;
            }
        }else if(this.cantidadKwh()>250 && this.cantidadKwh()<=400){
             if(medidor.getTipo().equals(medidor.getTipo().Comercial)){
                return (float)(((this.cantidadKwh()-250)*(1.5*40))+(250*(1.5*30)));
            }else{
                return (((this.cantidadKwh()-250)*40)+(250*30));
            }
        }else if(this.cantidadKwh()>400){
              if(medidor.getTipo().equals(medidor.getTipo().Comercial)){
                return (float)(((this.cantidadKwh()-400)*(1.5*50))+(250*(1.5*30))+(150*(1.5*40)));
            }else{
                return ((this.cantidadKwh()-400)*50)+(150*40)+(250*30);
            }
        }
        return 0;
    }
    public float descuentoPorGenero(){
        if(cliente.getGenero().equals(cliente.getGenero().Femenino)){
